 ...
 * 表中一般有两列：
 * 插入时间：例如create_time
 * 最后更新时间：例如update_time
 * <p>
 * 该插件使得：
 * 自动生成的insert语句：create_time & update_time总是填入数据库当前时间
 * 自动生成的update语句：不会包含create_time列，update_time总是填入数据库当前时间
 * <p>
 * 所有表中的插入时间列名，配置在createTimeColumns属性，以','分隔
 * 所有表中的最后更新时间列名，配置updateTimeColumns属性，以','分隔
 * 数据库当前时间表达式，配置dbCurrentTimeExpr
 * <p>
 * 示例
 * <plugin type="CreateUpdateTimePlugin">
 * <property name="createTimeColumns" value="create_time" />
 * <property name="updateTimeColumns" value="update_time" />
 * <property name="dbCurrentTimeExpr" value="current_timestamp" />
 * </plugin>
 *
 * @author PD
 */
public class CreateUpdateTimePlugin extends PluginAdapter {
    ...
    public boolean validate(List<String> warnings) {
        CommonUtils.splitByComma(properties.getProperty("createTimeColumns"), insertTimeColumnNames);
        CommonUtils.splitByComma(properties.getProperty("updateTimeColumns"), lastUpdateTimeColumnNames);
